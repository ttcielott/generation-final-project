...
def lambda_handler(event, context):
    ...
    try:
        for msg_no, msg in enumerate(event['Records']):
            print(f'lambda_handler: msg_no = {msg_no} and msg = {msg}')
            message_body = msg['body']
            msg_body_json = json.loads(message_body)
            print(f'lambda_handler: msg_body_json = {msg_body_json}')
            ...
            queue_url = 'https://sqs.eu-west-1.amazonaws.com/015206308301/mocha-madness-redshift-loader-queue'
            message_json = {'bucket': bucket, 'key': key}
